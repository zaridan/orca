...
} from './checks-panel-gitlab-review-rpc'
import type {
  ChecksAgentComposerState,
  ChecksPanelInnerProps,
  HostedReviewCreationSnapshot
} from './checks-panel-inner-types'
...
export function ChecksPanelInner({
  ...
  // ── Empty state ──
  // Why: equivalent to `!activeWorktree` for the active-worktree wrapper (repo is
  // null whenever the worktree is), but lets a shipped-PR caller drive this UI
  // from a repo/branch identity without a live worktree (PR2: Mission Control).
  if (!activeWorktree && !repo) {
    return (
